	...
	*/
	...
	public function fetch($tpl = null)
	{
		...
		if (! $result || $this->isError($result)) {
			...
			var_dump($result);
			echo '</pre>';
			//ExceptionHandler::handle($result->trace);
	...
	protected function applyFilters($buffer)
	{
	   $autoload = $this->__config['autoload'];
		foreach ($this->__config['filters'] as $callback) {
			...
			$callback[0] = '\Savant\Savant3\Filters\\' . $callback[0];
	...
	*/
		...
		$trace = true)
	{
		...
		if ($this->__config['exceptions']) {
			throw new \Savant\Savant3\Savant3_Exception($code);
		}
		...
		// return it
		$err = new \Savant\Savant3\Savant3_Error($config);
		return $err;
	}
	...
	*/
	...
	public function isError($obj)
	{
		...
		if (! is_object($obj)) {
			...
			$is = $obj instanceof \Savant\Savant3\Savant3_Error;
